...
def datagen(frames, mels):
    ...
    mark_x1 = min(all_x1)
    mark_x2 = max(all_x2)
    mark = [mark_y1, mark_y2, mark_x1, mark_x2]
    print(mark)
    for i, m in enumerate(mels):
        ...
        if len(img_batch) >= args.wav2lip_batch_size:
            ...
            yield img_batch, mel_batch, frame_batch, coords_batch, mark
            img_batch, mel_batch, frame_batch, coords_batch = [], [], [], []
...
def main():
    ...
    for i, (img_batch, mel_batch, frames, coords, mark) in enumerate(tqdm(gen,
                                                                    total=int(
        ...
        if i == 0:
            ...
            mark_out = cv2.VideoWriter('temp/face_bbox.avi',
                                  cv2.VideoWriter_fourcc(*'DIVX'), fps, (mark[3]-mark[2], mark[1]-mark[0]))
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            f[y1:y2, x1:x2] = p
            out.write(f)
            mark_frame = f[mark[0]:mark[1], mark[2]:mark[3]]
            relative_y1 = y1 - mark[0]
            relative_y2 = y1 - mark[0] + (y2 - y1)
            relative_x1 = x1 - mark[2]
            relative_x2 = x1 - mark[2] + (x2 - x1)
            mark_frame[relative_y1:relative_y2, relative_x1:relative_x2] = p
            mark_out.write(mark_frame)
